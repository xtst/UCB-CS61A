...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided, goal=GOAL_SCORE):
    ...
    if num_rolls != 0:
        ...
        ans = hefty_hogs(player_score, opponent_score)
    return ans
    # END PROBLEM 3
...
def announce_lead_changes(score0, score1, last_leader=None):
    ...
    if score0 > score1:
        ...
        now_leader = None
    if score0 != score1:
        string = f"Player {now_leader} takes the lead by {abs(score0-score1)}"
    else:
        string = None
    # if now_leader != last_leader:
    #     return (now_leader,)
    # else:
    #     return now_leader, None
    if now_leader != last_leader:
        return now_leader, string
    else:
        return now_leader, None
    # END PROBLEM 6
...
def hefty_hogs_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    """
    # BEGIN PROBLEM 10
    ans = hefty_hogs(score, opponent_score)
    if ans >= threshold:
        return 0
    else:
        return num_rolls
...
def hog_pile_strategy(score, opponent_score, threshold=8, num_rolls=6):
    ...
    if hog + hefty >= threshold or hog > 0:
        return 0
    else:
        return num_rolls
    # END PROBLEM 11
